...
export default function StatsScreen({ navigation }) {
  const { expenses, budget, getTotalSpent, currency, categories, colors } = useExpenses(); // <--- GET COLORS
  ...
  if (percentageUsed > 0.5) ringColor = (opacity = 1) => `rgba(255, 193, 7, ${opacity})`;
  if (percentageUsed > 0.9) ringColor = (opacity = 1) => `rgba(244, 67, 54, ${opacity})`;

  // 2. Pie Data
  const palette = ['#6C63FF', '#FF6584', '#FFC75F', '#4BC0C0', '#845EC2', '#FF9671', '#008F7A'];
  
  const pieData = categories.map((cat, index) => {
    const totalForCat = expenses.filter(e => e.category === cat).reduce((sum, item) => sum + item.amount, 0);
    ...
    return { name: cat, amount: totalForCat, color: sliceColor, legendFontColor: "#7F7F7F", legendFontSize: 12 };
  }).filter(item => item.amount > 0); 

  pieData.sort((a, b) => b.amount - a.amount);

  const handlePress = (name) => { setSelectedCategory(selectedCategory === name ? null : name); };
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top', 'left', 'right']}>
      ...
      <View style={styles.header}>
        <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backBtn}>
            <IconButton icon="arrow-left" size={28} iconColor={colors.text} style={{marginLeft: -10}} />
        </TouchableOpacity>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {/* BUDGET CARD */}
        <Surface style={[styles.budgetCard, { backgroundColor: colors.surface }]} elevation={3}>
            <View style={styles.ringRow}>
                ...
                <View style={styles.budgetInfo}>
                    <Text style={[styles.budgetLabel, { color: colors.textSec }]}>Monthly Budget</Text>
                    <Text style={[styles.budgetValue, { color: colors.text }]}>{(percentageUsed * 100).toFixed(0)}% Used</Text>
                    <View style={[styles.barContainer, { backgroundColor: colors.inputBg }]}>
        ...
        <Surface style={[styles.chartCard, { backgroundColor: colors.surface }]} elevation={2}>
            {pieData.length > 0 ? (
                <>
                    <View style={styles.chartWrapper}>
                        ...
                        <View style={[styles.donutHole, { backgroundColor: colors.surface }]}>
                            ...
                            <Text style={[styles.donutAmount, { color: colors.text }]}>{currency}{centerAmount.toLocaleString()}</Text>
                        </View>
                    </View>

                    <Divider style={{ marginVertical: 15, backgroundColor: colors.border }} />
                    <Text style={[styles.hintText, { color: colors.textSec }]}>Tap a category below to highlight 👆</Text>
                        ...
                        {pieData.map((item) => {
                            ...
                            return (
                                <TouchableOpacity 
                                    ...
                                    style={[styles.listItem, isSelected && { backgroundColor: colors.inputBg }, { borderBottomColor: colors.border }]}
                                >
                                    <View style={{flexDirection: 'row', alignItems: 'center'}}>
                                        <View style={[styles.dot, { backgroundColor: item.color }]} />
                                        <Text style={[styles.listName, { color: colors.text }, isSelected && { fontWeight: 'bold' }]}>{item.name}</Text>
                                    </View>
                                    <View style={{alignItems: 'flex-end'}}>
                                        <Text style={[styles.listAmount, { color: isSelected ? item.color : colors.text }]}>
                                            -{currency}{item.amount.toLocaleString()}
                                        </Text>
                                        <Text style={[styles.listPercent, { color: colors.textSec }]}>
                ...
                <View style={styles.emptyState}>
                    <IconButton icon="chart-donut" size={40} iconColor={colors.border} />
                    <Text style={{color: colors.textSec}}>No expenses yet this month.</Text>
                </View>
            )}
        </Surface>
...
const styles = StyleSheet.create({
  ...
  barContainer: { height: 6, borderRadius: 3, width: '100%', overflow: 'hidden' },
  ...
  donutHole: { position: 'absolute', width: 140, height: 140, borderRadius: 70, justifyContent: 'center', alignItems: 'center', zIndex: 10, pointerEvents: 'none' },
  donutLabel: { fontSize: 12, fontWeight: '600', marginBottom: 2, textTransform: 'uppercase' },
  donutAmount: { fontSize: 24, fontWeight: '800' },

  hintText: { textAlign: 'center', fontSize: 12, marginBottom: 10, fontStyle: 'italic' },

  listContainer: { paddingHorizontal: 10 },
  listItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 14, borderBottomWidth: 1, borderRadius: 12, paddingHorizontal: 10 },
